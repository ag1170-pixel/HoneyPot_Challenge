...
def validate_api_key(api_key: Optional[str] = Header(None, alias="x-api-key")) -> str:
    # For now, accept any key or no key (you can tighten this later)
    ...
    def handle_message(self, request: HoneypotRequest):
        try:
            ...
            response = {
                ...
                "is_scam": False,  # Simple logic for now
                "confidence": 0.5
            }
            
            # Try to send callback (optional)
            try:
...
# Vercel serverless function handler - No external dependencies
def handler(event, context):
    """
    Vercel serverless function handler
    """
    try:
        # Simple request parsing for Vercel
